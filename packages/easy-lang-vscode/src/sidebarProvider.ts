...
import { getI18nKeyStatus, getI18nKeyStatusWithLocation, KeyLocation } from './scanner';
...
export class I18nSidebarProvider implements vscode.TreeDataProvider<I18nTreeItem> {
  private _onDidChangeTreeData: vscode.EventEmitter<I18nTreeItem | undefined | void> = new vscode.EventEmitter<I18nTreeItem | undefined | void>();
  readonly onDidChangeTreeData: vscode.Event<I18nTreeItem | undefined | void> = this._onDidChangeTreeData.event;
  ...
  async refreshData() {
    ...
    const { untranslated, translated } = await getI18nKeyStatusWithLocation(this.workspaceRoot, this.translationPath);
    ...
    vscode.window.withProgress({
      ...
      for (const key of uniqueKeys) {
        ...
        for (const lang of targetLangs) {
          try {
            const translated = await autoTranslate(key, { ...options, to: lang });
...
export type I18nTreeItemType = 'untranslatedRoot' | 'translatedRoot' | 'untranslated' | 'translated';
...
export class I18nTreeItem extends vscode.TreeItem {
  ...
  constructor(label: string, type: I18nTreeItemType, key?: string, commandId?: string) {
